...
class WatchListViewController: UIViewController {
    ...
    private func setUpSearchController() {
        ...
        let resutVc = SearchResultViewController()
        resutVc.delegate = self
        let searchVC = UISearchController(searchResultsController: resutVc)
        searchVC.searchResultsUpdater = self
        navigationItem.searchController = searchVC

    }
    
    /// Set up custom tilte view
    ...
    private func createViewModels() {
        ...
        for(symbol , candleSticks ) in watchlistMap {
            ...
            viewModels.append(
                .init(symbol: symbol, companyName: UserDefaults.standard.string(forKey: symbol) ?? "Company", price: getLatestClosingPrice(from: candleSticks),
...
extension WatchListViewController : UISearchResultsUpdating {
    ...
    func updateSearchResults(for searchController: UISearchController) {
        guard let query = searchController.searchBar.text ,
                let resultsVc = searchController.searchResultsController as? SearchResultViewController , !query.trimmingCharacters(in: .whitespaces).isEmpty else {
            return
        }
       // print(query)
        //Call API to search
...
extension WatchListViewController: FloatingPanelControllerDelegate {
    ...
    func floatingPanelDidChangeState(_ fpc: FloatingPanelController) {
        navigationItem.titleView?.isHidden = fpc.position == .full
