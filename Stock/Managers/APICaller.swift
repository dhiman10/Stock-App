...
final class APICaller {
    ...
    private struct Constants {
        static let apiKey = "cffutrhr01qgfjm42kh0cffutrhr01qgfjm42khg"
        static let sandboxApiKey = "sandbox_c3c6me2ad3iefuuilmsg"
        static let baseUrl = "https://finnhub.io/api/v1/"
        static let day: TimeInterval = 3600 * 24
    }
    
//https://finnhub.io/api/v1/search?q=Apple&token=cffutrhr01qgfjm42kh0cffutrhr01qgfjm42khg
    /// Private constructor
    private init() {}
    ...
    public func search (query : String, completblock : @escaping (Result<SearchResponse, Error>)-> Void ) {
        
        guard let safeQuery = query.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)
        else {
            return
        }
    ...
    public func news(for type : NewsViewController.`Type`, completion : @escaping(Result<[NewsStory], Error>) -> Void){
        ...
        case .topStories :
            request(url: url(for: .topStories, queryParams: ["category": "general"]), expecting: [NewsStory].self, compeltion: completion)
    ...
    public func financialMetrics(for symbol : String, completion :@escaping(Result<FinancialMetricsResponse , Error>)-> Void ){
        ...
        request(url: url(for: .financials,queryParams: ["symbol": symbol, "metric": "all"]),
        ...
        let task = URLSession.shared.dataTask(with: url) { data, _ , error in
            ...
            guard let data = data, error == nil else {
                ...
                if let error = error {
                    ...
                    compeltion(.failure(APIError.noDataReturned))
                }
                return
            }
